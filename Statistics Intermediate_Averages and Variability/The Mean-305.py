...
sum_above = sum([i-mean for i in distribution if i > mean])
sum_below = sum([mean-i for i in distribution if i < mean])
...
for i in range(5000):
    ...
    sum_above = round(sum([i-mean for i in d if i > mean]), 1)
    sum_below = round(sum([mean-i for i in d if i < mean]), 1)
