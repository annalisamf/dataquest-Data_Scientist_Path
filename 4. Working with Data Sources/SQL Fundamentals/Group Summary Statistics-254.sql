...
SELECT Major_category, SUM(Women) Total_women, AVG(ShareWomen) Mean_women, SUM(Total)*AVG(ShareWomen) Estimate_women
FROM recent_grads
GROUP BY Major_category;

## 7. Multiple Group Columns ##

SELECT Major_category, Sample_category, AVG(ShareWomen) Mean_women, SUM(Total) Total_graduates 
FROM new_grads 
